...
/*fields: [{
  key: 'Prioridade', 'Cliente','Action',
  label: 'Prioridade', 'Cliente','Action',
  sortable:true
}*/
  ...
  vuejsDatepicker
    ...
    /*      if(cond=='prioridade'){
        ...
        while(i<msg.payload.length){
          app.items= [{ Prioridade: msg.payload[i].prioridade, Cliente: msg.payload[i].cliente, isActive: true },
           ]
            i++;
      }
   }*/
    ...
    methods: {
            ...
            teste2: function(event){

              console.log('bh')


            },
            baixar_prioridade: function(data) {
              var id = (data+1);
              var quantidade = this.items.length
              if (id<quantidade) {
              var id_cima = (data+2);
              var cliente = this.items[data+1].Cliente;
              var cliente_cima = this.items[data].Cliente;
              console.log("Descer prioridade " + id + "do cliente " + this.items[data].Cliente)
              console.log("Subir prioridade " + id + "do cliente " + this.items[data+1].Cliente)


                uibuilder.send( {
                    'topic': 'uibuilder/vue',
                    'payload': {
                        'type': 'descer_prioridade',
                        'id': id,
                        'cliente' : cliente,
                        'id_cima' : id_cima,
                       'cliente_cima' : cliente_cima
                    }
                } )
              }
            ...
            subir_prioridade: function(data) {
              var id = (data+1);
              if (id >=2) {


              var id_cima = (data);
              var cliente = this.items[data].Cliente;
              var cliente_cima = this.items[data-1].Cliente;
              console.log("Subir prioridade " + id + "do cliente " + this.items[data].Cliente)
              console.log("Descer prioridade " + id + "do cliente " + this.items[data-1].Cliente)

                uibuilder.send( {
                    'topic': 'uibuilder/vue',
                    'payload': {
                        'type': 'subir_prioridade',
                        'id': id,
                        'cliente' : cliente,
                        'id_cima' : id_cima,
                       'cliente_cima' : cliente_cima
    ...
    mounted: function(){
        ...
        // newVal relates to the attribute being listened to.
         ...
         if(cond == 'sensor'){
             app.sensor1 = msg.payload[0].sensor1
             app.sensor2 = msg.payload[0].sensor2
         }

         if(cond=='prioridade'){

            var i=0
            var prioridades=[msg.payload.length]
            var clientes = [msg.payload.length]
            var frase=[msg.payload.length]
           while(i<msg.payload.length){
                prioridades[i]=msg.payload[i].prioridade
                clientes[i]=msg.payload[i].cliente
                frase[i] = { Prioridade:prioridades[i], Cliente:clientes[i], isActive: true},
               i++;
         }
          // Create the dyanmic table according to the database
           app.items = frase
